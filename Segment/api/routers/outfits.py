    ...
    try:
        ...
        if not file_content:
            raise HTTPException(
                status_code=400, detail="Uploaded file is empty")
        prompt = """
        Provided below optionally is the image of a person, based on Color Therapy principles, suggest them outfits from the clothes data given below. Suggestion must only include the ID's of clothes.
